...
class TestCloudsFolder(TransactionCase):
    ...
    def _cron_trigger_check(self):
        ...
        Extra info:
         * IMPORTANT: do not launch the cron with method_direct_trigger since it will lose the context
        """
        self.env["sync.model"].with_context(cloud_testing=True).action_prepare_auto_folders()
