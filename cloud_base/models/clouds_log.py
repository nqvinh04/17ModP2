...
class clouds_log(models.Model):
    ...
    name = fields.Datetime(string="Log time")
    ...
    @api.model
    def action_prepare_logs_html(self, log_args={}, search_name=False, selected_clients=[], log_levels=[]):
        """
         ...
         * search_name - char (search by content)
         * selected_clients - list of integers
         * log_levels - list of chars
        ...
        """
        log_ids = self._search_logs_by_params(log_args, search_name, selected_clients, log_levels, True)
        ...
        return {
            "log_html": log_html,  "last_log": last_log, "first_log": first_log, "load_more_btn": load_more_btn,
        }

    @api.model
    def action_get_cloud_clients(self, selected_clients=[]):
        """
        ...
        if client_ids:
            result += client_ids.mapped(lambda cli: {
                ...
                "chosen": cli.id in selected_clients,
            })
        return result

    @api.model
    def action_get_cloud_queue(self, selected_clients=[]):
        """
        The method to retrieve the list of confirmed cloud clients and their states
        ...
        s_domain = []
        if selected_clients:
            selected_clients_int = [cli for cli in selected_clients if isinstance(cli, int)]
        ...
        return {"active_tasks": active_tasks, "failed_tasks": failed_tasks}

    @api.model
    def action_open_tasks(self, selected_clients=[], only_blocked=False):
        """
        The method to open clouds queue related to selected clients
        ...
        if selected_clients:
            selected_clients_int = [int(cli) for cli in selected_clients if str(cli).isdigit()]
            s_domain += [("client_id", "in", selected_clients_int)]
    ...
    @api.model
    def _prepare_txt_logs(self, search_name=False, selected_clients=[], log_levels=[]):
        """
         ...
         * search_name - char (search by content)
         * selected_clients - list of integers
         * log_levels - list of strings

        Returns:
         * text
        """
        log_ids = self._search_logs_by_params({}, search_name, selected_clients, log_levels, False)
        log_text = ""
        for log in log_ids:
    ...
    @api.model
    def _search_logs_by_params(
        self, log_args={}, search_name=False, selected_clients=[], log_levels=[], search_limit=False
    ):
        """
                ...
                domain += [
                    "|",
                        ...
                        "&", # to make sure simultaneous logs would appear
                            ("name", "=", last_object.name), ("id", ">", last_object.id)
                ]
            ...
            if first_object:
                domain += [
                    "|",
                        ...
                        "&", # to make sure simultaneous logs would appear
                            ("name", "=", first_object.name), ("id", "<", first_object.id)
                ]
        if search_name:
            domain += [
                "|", "|", "|", "|",
                    ("logs", "ilike", search_name),
                    ("name", "ilike", search_name),
                    ("log_type", "ilike", search_name),
                    ("client_name", "ilike", search_name),
                    ("client_id", "ilike", search_name),
            ]
        if selected_clients:
            domain.append(("client_id", "in", selected_clients))
        if log_levels:
            domain.append(("log_type", "in", log_levels))
        if search_limit:
