...
class Accounting_reportPartner_ledger(models.TransientModel):
    ...
    initial_balance = fields.Boolean(string='Include Initial Balance',
    ...
    def print_partner_ledger(self):
        ...
        used_context = {'lang': 'en_US', 'strict_range': True, 'date_from': self.date_from,
                        ...
                        'state': self.target_move, 'reconciled': self.reconciled,'initial_balance':self.initial_balance}
        query_get_data = self.env['account.move.line'].with_context(used_context)._where_calc(domain).get_sql()
        ...
        for partner_id in partner_ids:
            domain = [('partner_id', '=', partner_id), ('company_id', '=', self.env.company.id)]
            ...
            initial_debit = sum(line.debit for line in account_move_lines)
            initial_credit = sum(line.credit for line in account_move_lines)
            initial_balance = initial_debit - initial_credit

            partner_name = account_move_lines and account_move_lines[0].partner_id.name or ''
            partner_ledger_data = {
        ...
        if self._context.get('report_type') != 'excel':
            ...
            for partner in docs:
                ...
                if self.initial_balance :
                    final_data=self.env['report.bi_partner_ledger_report.bi_report_partnerledger'].with_context(
                       used_context=used_context)._calculate_initial_balancne(data, partner,'final_row')
                    if final_data: 
                        for line in final_data:
                            worksheet.write(row, col + 0, line['lname'], style=style_table_header)
                            worksheet.write(row, col + 4, line['debit'], style=style_table_header)
                            worksheet.write(row, col + 5, line['credit'], style=style_table_header)
                            worksheet.write(row, col + 6, line['initial_balance'], style=style_table_header)
                            row +=1
                for line in self.env['report.bi_partner_ledger_report.bi_report_partnerledger'].with_context(
                        reconciled=self.reconciled, used_context=used_context)._lines(data, partner):
                    if row > 50000:
                        count += 1
