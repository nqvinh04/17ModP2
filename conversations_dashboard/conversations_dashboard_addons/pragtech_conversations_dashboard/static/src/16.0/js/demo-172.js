odoo.define('pragtech_conversations_dashboard.demojs', function (require) {
   'use strict';
    console.log("demo-17.js has been loaded")
    var demo = $( document ).ready(function() {
        ...
        function handleleftheadertheme() {
            $('.theme-color > a.leftheader-theme').on("click", function() {
                console.log("handleleftheadertheme")
    ...
    /*  collapseable Left Header Change Function Start here*/
       ...
       handlecollapseLeftHeader ();
        $('.scoop-name-menu').on('click',function(ev){
            var $target = $(ev.currentTarget);
            var content_id = $target.attr('content');
            $('.panel-body .li-content').hide();
            $(content_id).show();
            console.log($(content_id));

        });

        $('.scoop-hasmenu').on('click', function(ev){
            var $target = $(ev.currentTarget);
            $target.find('ul li:first').trigger('click');
        });

        $('.panel-body .li-content').hide();
        $('.scoop-item').find('li').first().trigger('click');
        $('.scoop-trigger').find('ul li:first').trigger('click');

        $('body , .scoop-submenu').on('click', function(ev){
            ev.preventDefault();
            ev.stopImmediatePropagation();
            ev.stopPropagation();
        });

        $('.sidebar_toggle a').on('click', function(ev){
            $('.scoop-hasmenu.active').find('a').trigger('click');
