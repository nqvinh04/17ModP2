...
import { WhatsappMessageSeenIndicator } from "@pragtech_conversations_dashboard/dashboard_v17/message_seen_indicator";
 ...
 */
export class WhatsappMessage extends Component {
    ...
    static template = "whatsapp.Message";
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            // if (this.props.message.body.includes('http')){
            //     var rawText = this.props.message.body.replace('<p>', '');
            //     rawText = rawText.replace('</p>', '');
            //     this.props.message.body = `<a href='${rawText}'>"+${this.props.message.body}+"</a>`
            // }
    ...
    get rawmessage() {
        //        console.log("message2========", this.props.message)
                var rawText = this.props.message.body.replace('<p>', '');
                rawText = rawText.replace('</p>', '');
                var rawMsg = rawText.split('amp;');
                var rawMsg1 = ''
                for (let index = 0; index < rawMsg.length; index++) {
                    rawMsg1 = rawMsg1 + rawMsg[index];
                }
                // var rawMsg1 = rawMsg.replace('amp;', '');
                return rawMsg1;
            }
